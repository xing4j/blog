...
| ef | 非唯一索引等值查询，可能多行 | WHERE status = 'active' |
| ange | 索引范围查询 | WHERE id BETWEEN 1 AND 100 |
...
**诊断**：	ype=ALL，key=NULL，扫描 98 万行。create_time 没有索引。
