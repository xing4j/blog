...
Stream API 让集合操作变得优雅简洁，但它也暗藏不少陷阱：orEach 的变量修改不生效、parallel() 用错反而更慢、latMap 中的 
...
**8 个核心坑速记**：
1. orEach 内不要修改外部状态，用 collect 收集
2. Stream 只能消费一次
3. parallel() 不是万能提速，IO 操作禁止用默认 ForkJoinPool
4. latMap 返回值不能为 null，用 Stream.empty() 代替
5. orElse 总是求值，有开销时用 orElseGet
6. 	oMap 有重复 key 必须提供 merge function
7. peek 仅用于调试，勿做业务副作用
8. sorted/distinct 大数据量有内存压力，考虑提前处理或在 DB 层做

**延伸阅读方向**：
- Collectors 进阶：groupingBy、partitioningBy、	eeing（JDK 12+）
